...
BIGQUERY_TABLE = os.getenv("BIG_QUERY_META_TEBLE")
...
def fetch_all_ads_status():
    ...
    while True:
        try:
            response = session.get(url, timeout=60)  # Timeout 60 seg
            data = response.json()
...
def process_insight(insight, ads_status_map):
    try:
        ...
        # Inicializar el contador de compras
        purchases = 0

        # Extraer acciones y contar las compras
        ...
        return {
            ...
            "purchases": purchases,  # Nuevo campo para el número de compras
            "status": status,
...
def extract_insights_meta(days_back=2):
    ...
    base_url = (
        ...
        f"impressions,clicks,ctr,spend,actions"
    ...
    # 5) Procesar con 2 niveles de batch
    buffer = []
    new_records = 0
